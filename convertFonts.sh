...
USE_FONT_PATH_PREFIX=`echo "$ARGS" | grep '\--use-font-path-prefix'`
...
FONT_PATH_PREFIX="`echo $USE_FONT_PATH_PREFIX | awk -F'=' '{print $2}'`"
...
do
	...
	if [ "$IS_OTF" = "0" ]
	then
		EXTRA_FONT_INFO=" url('$FONT_PATH_PREFIX$i.otf')  format('opentype'),
      "
	else
		EXTRA_FONT_INFO=""
	fi
...
@font-face {
  ...
  src: url('$FONT_PATH_PREFIX$i.eot?') format('eot'),"

if [ "$HAS_WOFF2_COMPRESS" = "0" -a -f "$i.woff" ]
then
	RULE="$RULE
       url('$FONT_PATH_PREFIX$i.woff2') format('woff2'),"
fi

RULE="$RULE
       url('$FONT_PATH_PREFIX$i.woff') format('woff'),
      $EXTRA_FONT_INFO url('$FONT_PATH_PREFIX$i.ttf')  format('truetype');
...
@font-face {
  font-family: '$FONTNAME';
  src: url('$FONT_PATH_PREFIX$i.eot?') format('eot')," >> $STYLESHEETFILE

  if [ "$HAS_WOFF2_COMPRESS" = "0" -a -f "$i.woff2" ]
  then
		echo "       url('$FONT_PATH_PREFIX$i.woff2') format('woff2')," >> $STYLESHEETFILE
  fi

echo "       url('$FONT_PATH_PREFIX$i.woff') format('woff'),
      $EXTRA_FONT_INFO url('$FONT_PATH_PREFIX$i.ttf')  format('truetype'),
       url('$FONT_PATH_PREFIX$i.svg#$SVG_ID') format('svg');
}" >> $STYLESHEETFILE
  fi
done
echo "DONE!"
